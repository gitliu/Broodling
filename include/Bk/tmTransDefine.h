...
typedef struct tmCompression_t
{
	...
	//unsigned int	dwSize;
	//视频编码格式，0-BKMPEG4,1-H264,2-MJPEG,10-YPbPr,11-cvbs,12-hdmi
...
//云台配置信息
typedef struct tmPtzCfg_t
{
	unsigned int	dwSize;					//本结构大小
	unsigned char	byEnableKeepWatch;		//是否启用看守位0,1
	unsigned char	byRunMode;				//运行模式0-手动运行，1-自动运行，2-按时间布防运行
	unsigned char	byRunType;				//运行类型0-巡航，1-轨迹
	unsigned char	byRunLine;				//运行的线,0xFF为所有线	
	unsigned int	dwKeepWatchCheckTime;	//(秒)停止云台控制一定时间后开始启用调用看守位
											//或者在系统启动后调用看守位
	tmSchedTime_t	struSchedTime[MAX_DAYS][MAX_TIMESEGMENT];
...
typedef struct tmVideoInCfg_t
{
	...
	//白平衡控制0-自动,1-白炽灯,2-D4000,3-D5000,4-日光,5-阴天,6-闪光灯,7-荧光的,8-荧光的H,9-水下,0xFF-手动
...
#define TMCC_MINOR_CMD_SERIALRECV					0x09		//服务器返回的串口数据
...
#define TMCC_MINOR_CMD_AUTOFOCUSBALANCE				0x11		//自动查找Focus的值
